...
def capture_image_from_cam_into_temp(sign=1):
    ...
    while True:
        ...
        if k % 256 == 27:
            # ESC pressed
            print("Escape hit, the camera is closing...")
            break
        elif k % 256 == 32:
            # SPACE pressed
...
def captureImage(ent, sign=1):
    ...
    res = messagebox.askquestion(
        'Click Picture', 'Press Space Bar to click picture and ESC to exit the camera')
    if res == 'yes':
...
def checkSimilarity(window, path1, path2):
    ...
    if(result <= THRESHOLD):
        messagebox.showerror("Failure: Signatures Do Not Match",
                             "Result: FORGED! Signatures are "+str(result)+f" % similar")
        pass
    else:
        messagebox.showinfo("Success: Signatures Match",
                            "Result: Original and Not forged! Signatures are "+str(result)+f" % similar")
    return True
...
uname_label = tk.Label(root, text="Compare the Signatures:", font=16)
uname_label.place(x=90, y=50)

img1_message = tk.Label(root, text="Signature 1", font=12)
...
img1_capture_button = tk.Button(
    root, text="Capture", font=12, command=lambda: captureImage(ent=image1_path_entry, sign=1))
img1_capture_button.place(x=400, y=90)

img1_browse_button = tk.Button(
    root, text="Browse", font=12, command=lambda: browsefunc(ent=image1_path_entry))
img1_browse_button.place(x=400, y=140)

image2_path_entry = tk.Entry(root, font=12)
image2_path_entry.place(x=150, y=240)

img2_message = tk.Label(root, text="Signature 2", font=12)
img2_message.place(x=10, y=250)

img2_capture_button = tk.Button(
    root, text="Capture", font=12, command=lambda: captureImage(ent=image2_path_entry, sign=2))
img2_capture_button.place(x=400, y=210)

img2_browse_button = tk.Button(
    root, text="Browse", font=12, command=lambda: browsefunc(ent=image2_path_entry))
img2_browse_button.place(x=400, y=260)

compare_button = tk.Button(
    root, text="Compare", font=18, command=lambda: checkSimilarity(window=root,
